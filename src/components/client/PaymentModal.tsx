...
import { CreditCard, Loader, CheckCircle, XCircle, X, Clock, AlertTriangle } from "lucide-react";
import { useLanguage } from "@/contexts/LanguageContext";
import { useInitiatePayment, useManualCheckPaymentStatus } from "@/hooks/usePaymentQuery";
...
export const PaymentModal: React.FC<PaymentModalProps> = ({
  ...
  const [paymentStatus, setPaymentStatus] = useState<'idle' | 'processing' | 'checking' | 'success' | 'failed' | 'timeout'>('idle');
    ...
    const interval = setInterval(() => {
      checkPaymentStatus({
        ...
        onSuccess: (response) => {
          ...
          } else if (response.payment_status === 'failed' || response.payment_status === 'cancelled') {
            clearInterval(interval);
            setStatusCheckInterval(null);
  ...
  const handlePayment = async () => {
    ...
    const paymentData = {
      ...
      currency: invoice.currency || 'SAR',
      order_id: newOrderId,
      description: invoice.service_description || 'Visa Service Payment',
      ...
      <DialogContent className="max-w-md" dir={language === "ar" ? "rtl" : "ltr"}>
        ...
        <div className="space-y-6">
          ...
          <div className={`bg-gray-50 p-4 rounded-lg ${language === "ar" ? "text-right" : "text-left"}`}>
            <h3 className="font-semibold text-visa-dark mb-2">{t('paymentSummary')}</h3>
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                <span>{t('service')}:</span>
                <span className="font-medium">{invoice.service_description || t('visaServices')}</span>
              </div>
              <div className="flex justify-between">
                <span>{t('amount')}:</span>
                <span className="font-bold text-lg">{invoice.amount} {invoice.currency || 'SAR'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="text-center">
            ...
            {paymentStatus === 'checking' && (
              <div className="space-y-4">
                ...
                <p className="text-sm text-gray-500">{t('pleaseWaitPaymentVerification')}</p>
                {paymentUrl && (
                  <a
                    href={paymentUrl}
                    target="_blank"
            ...
            {paymentStatus === 'success' && (
              <div className="space-y-4">
                ...
                <p className="text-green-600 font-medium">{t('paymentSuccessful')}</p>
            ...
            {paymentStatus === 'timeout' && (
              <div className="space-y-4">
                ...
                <p className="text-orange-600 font-medium">{t('paymentVerificationTimeout')}</p>
                <p className="text-sm text-gray-600">{t('paymentMayStillBeProcessing')}</p>
