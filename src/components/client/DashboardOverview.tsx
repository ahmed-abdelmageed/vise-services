...
export const DashboardOverview = ({ user, applications = [], onSectionChange }: DashboardOverviewProps) => {
  ...
  const openRequests = applications?.length > 0 
    ...
    : [
        { id: "REQ-2023-045", title: t('visaStatusInquiry'), status: "In Progress", updatedAt: "2023-10-29" },
        { id: "REQ-2023-048", title: t('additionalDocumentSubmission'), status: "Pending", updatedAt: "2023-10-30" },
      ];
  ...
  return (
    <div className="space-y-6" dir={language === "ar" ? "rtl" : "ltr"}>
      <div className={`flex flex-col md:flex-row justify-between items-start md:items-center ${language === "ar" ? "md:flex-row-reverse" : ""}`}>
        
        <div className={`mt-4 md:mt-0 flex items-center gap-3 ${language === "ar" ? "flex-row-reverse" : ""}`}>
          ...
          <div className={language === "ar" ? "text-right" : "text-left"}>
            <p className={`font-semibold text-visa-dark ${language === "ar" ? "text-right" : "text-left"}`}>{user.name}</p>
            <p className={`text-sm text-gray-500 ${language === "ar" ? "text-left" : "text-left"}`}>{user.email}</p>
          </div>
        </div>
        <div className={language === "ar" ? "text-right" : "text-left"}>
          <h1 className={`text-2xl font-bold text-visa-dark ${language === "ar" ? "text-right" : "text-left"}`}>{t('welcome')} {user.name}</h1>
          <p className={`text-gray-500 mt-1 ${language === "ar" ? "text-right" : "text-left"}`}>{t('accountOverview')}</p>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className={`space-y-3 ${language === "ar" ? "text-right" : "text-left"}`}>
                ...
                <div className={`flex items-center gap-2 ${language === "ar" ? "flex-row-reverse" : ""}`}>
                  ...
                  <span className="text-sm opacity-90">{applications.length > 1 ? t('Applications') : t('Application')}</span>
                </div>
              </div>
              ...
              <div className={`space-y-3 ${language === "ar" ? "text-right" : "text-left"}`}>
                ...
                <div className="bg-white/20 rounded-lg px-3 py-2 backdrop-blur-sm">
                  <span className="text-lg font-medium">
                    {applications[0]?.service_type === "prepare-file-only" ? t('prepareFileOnly') : applications[0]?.service_type || "Visa Service"}
                  </span>
                </div>
              </div>
            </div>
      ...
      <div className={`grid grid-cols-1 md:grid-cols-3 gap-6 ${language === "ar" ? "" : ""}`} dir={language === "ar" ? "rtl" : "ltr"}>
          ...
          <CardContent>
              ...
              <div className="space-y-4">
                {activeServices.map((service) => (
                  <div key={service.id} className="border-b pb-3 last:border-b-0 last:pb-0">
                    <p className={`font-medium text-sm ${language === "ar" ? "text-right" : "text-left"}`}>{service.name}</p>
                    <div className={`flex justify-between items-center mt-1 ${language === "ar" ? "flex-row-reverse" : ""}`}>
                      ...
                      <span className={`text-xs text-gray-500 ${language === "ar" ? "text-right" : "text-left"}`}>Ref: {service.reference}</span>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <div className={`text-center py-6 ${language === "ar" ? "text-center" : "text-center"}`}>
            ...
            {activeServices.length > 0 && (
              <Button 
                variant="ghost" 
                size="sm" 
                className={`w-full mt-3 ${language === "ar" ? "flex-row-reverse" : ""} text-blue-600 hover:text-blue-700 hover:bg-blue-50 gap-2`}
        ...
        <Card className="bg-white hover:shadow-lg transition-shadow duration-200 border border-gray-100">
          ...
          <CardContent>
            <div className="space-y-4">
              {recentInvoices.map((invoice) => (
                <div key={invoice.id} className="border-b pb-3 last:border-b-0 last:pb-0">
                  <p className={`font-medium text-sm ${language === "ar" ? "text-right" : "text-left"}`}>{invoice.id}</p>
                  <div className={`flex justify-between items-center mt-1 ${language === "ar" ? "flex-row-reverse" : ""}`}>
                    <span className={`text-xs text-gray-500 ${language === "ar" ? "text-right" : "text-left"}`}>{invoice.date}</span>
                    <div className={`flex items-center gap-2 ${language === "ar" ? "flex-row-reverse" : ""}`}>
            ...
            <Button 
              variant="ghost" 
              size="sm" 
              className={`w-full mt-3 ${language === "ar" ? "flex-row-reverse" : ""} text-green-600 hover:text-green-700 hover:bg-green-50 gap-2`}
