...
export const useAdminDashboard = () => {
  ...
  }, [refetchCustomers, refetchInvoices, queryClient]);
  ...
  const handleDeleteCustomer = useCallback(async (id: string) => {
    try {
      ...
      toast.error(`An error occurred while deleting the customer: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }, [handleDataChanged]);
