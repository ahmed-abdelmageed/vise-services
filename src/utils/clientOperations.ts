    ...
    if (clientData.passport_files) {
      ...
      for (const file of passportFiles) {
        if (typeof file === 'string') {
          ...
          if (deleteFileError) {
            console.error(`Error deleting passport file ${file}:`, deleteFileError);
          }
        }
      }
    }
    ...
    if (clientData.photo_files) {
      ...
      for (const file of photoFiles) {
        if (typeof file === 'string') {
          ...
          if (deleteFileError) {
            console.error(`Error deleting photo file ${file}:`, deleteFileError);
          }
        }
      }
    }
...
export const bulkDeleteClients = async (clientIds: string[]): Promise<{ success: boolean; message: string }> => {
  try {
    if (!clientIds || clientIds.length === 0) {
      return { success: false, message: "No client IDs provided." };
    }

    console.log(`Attempting to delete clients with IDs: ${clientIds.join(', ')}`);
    ...
    const message = `An unexpected error occurred: ${error instanceof Error ? error.message : 'Unknown error'}`;
    toast.error(message);
...
export const fetchClientDocuments = async (clientId: string) => {
  try {
    ...
    toast.error(`Unexpected error loading documents: ${error instanceof Error ? error.message : 'Unknown error'}`);
    return [];
  }
};
...
export const fetchApplicationFiles = async (clientId: string) => {
  try {
    ...
    const { data, error } = await supabase
      .from('visa_applications')
      .select('passport_files, photo_files, created_at')
      .eq('id', clientId)
      .single();
    ...
    if (error) {
      ...
      return { passportFiles: [], photoFiles: [] };
    }
    ...
    console.log(`Retrieved ${passportFiles.length} passport files and ${photoFiles.length} photo files`);
    return { passportFiles, photoFiles };
  } catch (error) {
    console.error("Unexpected error fetching application files:", error);
    toast.error(`Unexpected error loading files: ${error instanceof Error ? error.message : 'Unknown error'}`);
    return { passportFiles: [], photoFiles: [] };
  }
};
...
export const bulkUpdateClientStatus = async (
  ...
  try {
    ...
    console.log(`Attempting to update status for clients with IDs: ${clientIds.join(', ')}`);
    ...
    if (error) {
      ...
      return { success: false, message: `Failed to update client status: ${error.message}` };
    }

    console.log(`Successfully updated status for clients with IDs: ${clientIds.join(', ')}`);
...
export const deleteFile = async (
  ...
  try {
    console.log(`Attempting to delete ${fileType} for client ${clientId}: ${fileUrl}`);
    ...
    if (currentFiles.length === updatedFiles.length) {
      console.error(`File ${fileUrl} not found in ${fileType} for client ${clientId}`);
      return false;
    }
    
    // Update the database with the new file array
    const updatePayload = { [fileType]: updatedFiles, updated_at: new Date().toISOString() };
    ...
    const storageUrl = "https://uspqgeozxmfmingjasro.supabase.co/storage/v1/object/public/visa-documents/";
    ...
    if (storageError) {
      ...
      console.warn(`File entry removed from database but file may still exist in storage: ${filePath}`);
    }
    
    console.log(`Successfully deleted ${fileType} for client ${clientId}: ${fileUrl}`);
    return true;
  } catch (error) {
